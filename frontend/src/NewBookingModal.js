...
export default function NewBookingModal({ open, onClose, onCreated, defaultDate, tables = [], areas = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" data-testid="new-booking-modal">
      <div className="w-full max-w-3xl bg-white rounded-lg shadow-2xl border border-zinc-200 max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input data-testid="nb-date" type="date" value={form.date} onChange={(e) => set("date", e.target.value)}
                   className="w-full border border-zinc-200 rounded-md px-3 py-2" required />
          </div>
          <div>
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {suggestion?.suggested && (
                <button type="button" onClick={() => set("table_ids", [suggestion.suggested])}
                        className="text-xs inline-flex items-center gap-1 text-zinc-600 hover:text-zinc-900" data-testid="nb-use-suggestion">
                  <Sparkles size={12} /> Suggerito
                </button>
              )}
            </div>
            <div className="flex flex-wrap gap-2 border border-zinc-200 rounded-md p-3 max-h-40 overflow-y-auto bg-zinc-50">
              {tables.length === 0 && <div className="text-xs text-zinc-500">Nessun tavolo configurato</div>}
              {tables.map((tb) => {
                  ...
                  <button type="button" key={tb.id} onClick={() => toggleTable(tb.id)}
                          ...
                          className={`px-3 py-1 rounded-md text-xs border transition-colors ${
                            isSelected ? "bg-zinc-900 text-white border-zinc-900"
                              : isCandidate ? "border-emerald-400 text-emerald-800 bg-emerald-50"
                              : "border-zinc-200 text-zinc-600 opacity-60"
                          }`}>
          ...
          <div className="md:col-span-2 flex items-center justify-end gap-3 pt-2 border-t border-zinc-100">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-md border border-zinc-200 text-zinc-700 hover:bg-zinc-50">
              {t("common.cancel")}
            </button>
            <button data-testid="nb-submit" type="submit" disabled={busy}
                    className="px-5 py-2 rounded-md bg-zinc-900 hover:bg-zinc-700 text-white font-semibold disabled:opacity-50 transition-colors">
              {busy ? "…" : t("common.save")}
            </button>
          </div>
        </form>
