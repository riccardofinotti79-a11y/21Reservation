...
export default function CustomerDetail() {
  ...
  if (!c) return <div className="p-8 text-zinc-400 dark:text-zinc-500">Caricamento…</div>;
  ...
  return (
    <div className="p-8 max-w-[1000px] mx-auto">
      <Link to="/customers" className="inline-flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 hover:text-zinc-900 dark:text-zinc-100 mb-4">
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          ...
          <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 mt-1 font-mono">{c.phone || "—"} · {c.email || "—"}</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-5 py-3">
          <div className="label-eyebrow">{t("customers.reliability")}</div>
          <div className={`text-3xl font-serif-display ${score === null ? "text-zinc-400 dark:text-zinc-500" : score >= 80 ? "text-emerald-700" : score >= 50 ? "text-amber-700" : "text-red-700"}`}>
            {score === null ? "—" : score}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-5 mb-6">
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 block mb-1">Tag (separati da virgola)</label>
            <input value={tags} onChange={(e) => setTags(e.target.value)} className="w-full border border-zinc-200 dark:border-zinc-800 rounded-md px-3 py-2" />
          </div>
          <div>
            <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 block mb-1">Note</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full border border-zinc-200 dark:border-zinc-800 rounded-md px-3 py-2" />
          </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden">
        <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 label-eyebrow">{t("customers.history")}</div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {bookings.length === 0 && <tr><td colSpan={4} className="p-6 text-center text-zinc-400 dark:text-zinc-500">—</td></tr>}
            {bookings.map((b) => (
