...
export default function Tables() {
  ...
  const renderMobileCard = (tb) => {
    ...
    if (isEditing) {
      return (
        <div key={tb.id} className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/40 space-y-2" data-testid={`table-row-${tb.name}`}>
          ...
          <select value={editingTable.shape} onChange={(e) => setEditingTable({ ...editingTable, shape: e.target.value })} className="w-full border rounded-md px-3 py-2 text-base bg-white dark:bg-zinc-900">
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <button onClick={() => setEditingTable(null)} className="min-h-[40px] min-w-[40px] p-2 rounded-md border border-zinc-200 dark:border-zinc-800 flex items-center justify-center"><X size={16} /></button>
          </div>
        </div>
      );
    }
    return (
      <div key={tb.id} className="p-4 border-t border-zinc-100 dark:border-zinc-800" data-testid={`table-row-${tb.name}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="font-medium text-lg">{tb.name}</div>
            <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-500 mt-1 uppercase tracking-widest">{tb.shape}</div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button data-testid={`btn-edit-table-${tb.name}`} onClick={() => setEditingTable({ ...tb })}
                    className="min-w-[40px] min-h-[40px] p-2 rounded-md border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center">
              <Pencil size={16} />
            </button>
        ...
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm">
          <div><span className="text-zinc-400 dark:text-zinc-500 text-[10px] uppercase tracking-widest mr-1">{t("tables.seats_min")}</span><span className="font-mono">{tb.seats_min}</span></div>
          <div><span className="text-zinc-400 dark:text-zinc-500 text-[10px] uppercase tracking-widest mr-1">{t("tables.seats_max")}</span><span className="font-mono">{tb.seats_max}</span></div>
          <div><span className="text-zinc-400 dark:text-zinc-500 text-[10px] uppercase tracking-widest mr-1">{t("tables.priority")}</span><span className="font-mono">{tb.priority}</span></div>
          <div><span className="text-zinc-400 dark:text-zinc-500 text-[10px] uppercase tracking-widest mr-1">{t("tables.bookable_online")}</span>{tb.bookable_online ? "✓" : "—"}</div>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="p-4 sm:p-8 max-w-[1400px] mx-auto">
      ...
      <div className="space-y-6">
        {grouped.length === 0 && (
          <div className="p-16 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-center text-zinc-400 dark:text-zinc-500">
            Nessuna area configurata. Creane una per iniziare.
          </div>
        )}
        {grouped.map(({ area, tables: ts }) => (
          <div key={area.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/60 flex-wrap gap-2">
              <div>
              ...
              <div className="flex items-center gap-2">
                <button data-testid={`btn-add-table-${area.name}`} onClick={() => startAddTable(area.id)}
                        className="inline-flex items-center gap-1 px-3 py-2 rounded-md border border-zinc-200 dark:border-zinc-800 text-sm hover:bg-white dark:bg-zinc-900">
            ...
            {addingTableTo === area.id && (
              <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/40 grid grid-cols-2 md:grid-cols-7 gap-2">
                <input data-testid="new-table-name" placeholder={t("common.name")} value={tableDraft.name}
                       ...
                       className="border border-zinc-200 dark:border-zinc-800 rounded-md px-2 py-2 text-base md:text-sm col-span-2 md:col-span-1" />
                <input type="number" min={1} placeholder={t("tables.seats_min")} value={tableDraft.seats_min}
                       ...
                       className="border border-zinc-200 dark:border-zinc-800 rounded-md px-2 py-2 text-base md:text-sm" />
                ...
                <select value={tableDraft.shape} onChange={(e) => setTableDraft({ ...tableDraft, shape: e.target.value })}
                        className="border border-zinc-200 dark:border-zinc-800 rounded-md px-2 py-2 text-base md:text-sm bg-white dark:bg-zinc-900">
                  {SHAPES.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
                ...
                <div className="flex items-center gap-2 col-span-2 md:col-span-1">
                  ...
                  <button onClick={() => setAddingTableTo(null)} className="min-w-[40px] min-h-[40px] p-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center"><X size={16} /></button>
                </div>
              </div>
            )}
            ...
            {isMobile ? (
              <div>
                {ts.length === 0 && <div className="px-4 py-8 text-center text-zinc-400 dark:text-zinc-500 text-sm">Nessun tavolo</div>}
                {ts.map((tb) => renderMobileCard(tb))}
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">
                    <tr>
                  ...
                  <tbody>
                    {ts.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-zinc-400 dark:text-zinc-500">Nessun tavolo</td></tr>}
                    {ts.map((tb) => {
                      const isEditing = editingTable?.id === tb.id;
                      return (
                        <tr key={tb.id} className="border-t border-zinc-100 dark:border-zinc-800" data-testid={`table-row-${tb.name}`}>
                          {isEditing ? (
                            <>
                              ...
                              <td className="px-3 py-2">
                                <div className="flex items-center justify-end gap-1">
                                  ...
                                  <button onClick={() => setEditingTable(null)} className="p-1.5 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"><X size={14} /></button>
                                </div>
                              </td>
                            </>
                          ) : (
                            <>
                              ...
                              <td className="px-4 py-2">
                                <div className="flex items-center justify-end gap-1">
                                  <button data-testid={`btn-edit-table-${tb.name}`} onClick={() => setEditingTable({ ...tb })} className="p-1.5 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"><Pencil size={14} /></button>
